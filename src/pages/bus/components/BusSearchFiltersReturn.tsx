...
const BusSearchFiltersReturn: React.FC = () => {
  const { returnTrips, tripFiltersReturn, setTripFiltersReturn, setSearchValues } = useSearchContext();
  const navigate = useNavigate();
  const [isModalOpen, setIsModalOpen] = useState(false);

  // Compute unique filter options based on raw trips
  const companyOptions = useMemo(() => {
    return Array.from(new Set(returnTrips.map((trip) => trip.company)));
  }, [returnTrips]);

  // Deduplicate departure stations by name
  const departureStationOptions = useMemo(() => {
    return Array.from(
      new Set(returnTrips.flatMap((trip) => trip.stations_from.map((s) => s.name)))
    );
  }, [returnTrips]);
  ...
  const arrivalStationOptions = useMemo(() => {
    return Array.from(
      new Set(returnTrips.flatMap((trip) => trip.stations_to.map((s) => s.name)))
    );
  }, [returnTrips]);

  // Compute min and max price range
  const priceRange = useMemo(() => {
    if (returnTrips.length === 0) return { min: 0, max: 0 };
    const prices = returnTrips.map((trip) => trip.price_start_with);
  ...
  const FilterSectionsContent = ({ alwaysOpen = false }: { alwaysOpen?: boolean }) => (
    <div className="grid grid-cols-1 gap-8">
      {/* Company Filter */}
      <FilterSection title="Companies" alwaysOpen={alwaysOpen}>
        {companyOptions.map((company) => (
          <FilterCheckbox
            key={company}
            label={company}
            checked={tripFiltersReturn.company === company}
            onChange={() =>
              handleFilterChange(
                "company",
                tripFiltersReturn.company === company ? "" : company
              )
            }
          />
        ))}
      ...
      <FilterSection title="Departure Stations" alwaysOpen={alwaysOpen}>
        {departureStationOptions.map((station) => (
          <FilterCheckbox
            key={station}
            label={station}
            checked={tripFiltersReturn.stationFrom === station}
            onChange={() =>
              handleFilterChange(
                "stationFrom",
                tripFiltersReturn.stationFrom === station ? "" : station
              )
            }
          />
        ))}
      ...
      <FilterSection title="Arrival Stations" alwaysOpen={alwaysOpen}>
        {arrivalStationOptions.map((station) => (
          <FilterCheckbox
            key={station}
            label={station}
            checked={tripFiltersReturn.stationTo === station}
            onChange={() =>
              handleFilterChange(
                "stationTo",
                tripFiltersReturn.stationTo === station ? "" : station
              )
            }
          />
        ))}
      ...
      <FilterSection title="Price Range" alwaysOpen={alwaysOpen}>
        <div className="flex flex-col gap-3 py-4">
          <label className="text-gray-700 text-sm">
            {`Price Range: ${tripFiltersReturn.priceMin ?? priceRange.min} - ${
              tripFiltersReturn.priceMax ?? priceRange.max
            } LE`}
          </label>
          <input
            type="range"
            min={priceRange.min}
            max={priceRange.max}
            value={tripFiltersReturn.priceMin ?? priceRange.min}
            onChange={(e) =>
          ...
          <input
            ...
            min={priceRange.min}
            max={priceRange.max}
            value={tripFiltersReturn.priceMax ?? priceRange.max}
            onChange={(e) =>
...
export default BusSearchFiltersReturn;
